...
import com.vvm.sh.R;
import com.vvm.sh.ui.informacaoSst.adaptadores.ObrigacaoLegalRecyclerAdaptor;
...
public class InformacaoSstBinding {

    @BindingAdapter({"obrigacoes" , "listener"})
    public static void setObrigacoesLegais(RecyclerView view, List<ObrigacaoLegal> items, OnInformacaoSstListener listener) {
        ...
        ObrigacaoLegalRecyclerAdaptor adapter = (ObrigacaoLegalRecyclerAdaptor) view.getAdapter();

        if(adapter == null){
            adapter = new ObrigacaoLegalRecyclerAdaptor(view.getContext(), items, listener);
